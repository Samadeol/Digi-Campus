...
function buildList() {
    ...
    url = 'http://127.0.0.1:8000/api/main_menu_list/';
    fetch(url)
        .then((resp) => resp.json())
        .then(function(data) {
            var list = data;
...
function update_main_list() {
    var url = 'http://127.0.0.1:8000/api/main_menu_create/';
...
function update_extra_list() {
    var url = 'http://127.0.0.1:8000/api/main_extras_create/';
    var name = document.getElementById('name').value;
    var price = document.getElementById('price').value;
    fetch(url, {
        method: 'POST',
...
function deleteItem(id) {
    fetch('http://127.0.0.1:8000/api/main_extras_delete/' + id, {
        method: 'DELETE',
        headers: {
            'Content-type': 'application/json',
            'X-CSRFToken': csrftoken,
